...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  DateTime? filterDate; // Added for specific date filtering
  ...
  List<dynamic> breakHistory = []; // Added for break report
  ...
  void initState() {
    ...
    filterDate = DateTime.now(); // Default to today
  ...
  Future<void> _fetchReport() async {
    ...
    try {
      ...
      String? dateStr;
      if (filterDate != null) {
        dateStr = "${filterDate!.year}-${filterDate!.month.toString().padLeft(2, '0')}-${filterDate!.day.toString().padLeft(2, '0')}";
      }
  ...
  void _processReportData(Map<String, dynamic> data) {
    ...
    } else if (selectedReportType == 'Break In') {
      breakHistory = records;
      // You can calculate total break duration here if summary is missing
    } else {
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
                  ...
                  children: [
                    Container(
                      ...
                      child: DropdownButtonFormField<String>(
                        ...
                        items: ['Attendance', 'Leave', 'Break In', 'Payroll']
                            .map(
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          Expanded(
                            child: InkWell(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text("Month View", style: GoogleFonts.poppins(fontSize: 10, color: Colors.grey)),
                                  Text(
                                    selectedMonth == null
                                        ? "Select Month"
                          ...
                          Container(width: 1, height: 30, color: Colors.grey.shade300, margin: const EdgeInsets.symmetric(horizontal: 10)),
                          Expanded(
                            child: InkWell(
                              onTap: () async {
                                ...
                                if (picked != null) {
                                  setState(() {
                                    filterDate = picked;
                                    selectedMonth = monthNames[picked.month - 1];
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text("Specific Date", style: GoogleFonts.poppins(fontSize: 10, color: Colors.grey)),
                                  Text(
                                    filterDate == null
                                        ? "Select Date"
                                        : "${filterDate!.day.toString().padLeft(2, '0')} ${monthNames[filterDate!.month - 1].substring(0, 3)} ${filterDate!.year}",
                                    style: GoogleFonts.poppins(
                                      fontSize: 14,
                                      fontWeight: FontWeight.w500,
                          ...
                          InkWell(
                            onTap: () {
                              setState(() {
                                ...
                                selectedMonth = monthNames[filterDate!.month - 1];
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(color: const Color(0xFF26A69A).withOpacity(0.1), borderRadius: BorderRadius.circular(4)),
                              child: Text("Today", style: GoogleFonts.poppins(fontSize: 10, color: const Color(0xFF26A69A), fontWeight: FontWeight.bold)),
                            ),
                          ),
                        ],
                      ),
              ...
              // ? Detailed History (Attendance or Leave or Break In)
              if (selectedReportType == 'Break In') ...[
                Text(
                  "Break In Report",
                  style: GoogleFonts.poppins(
                    fontSize: 17,
                ...
                ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    final item = breakHistory[index];
                    String purpose = item["reason"] ?? item["purpose"] ?? "Break";
                    String date = item["date"] ?? "";
                    String times = "In: ${item["break_in"] ?? "--"} | Out: ${item["break_out"] ?? "--"}";
                    String duration = "${item["duration_minutes"] ?? "0"} min";
                ...
                const SizedBox(height: 20),
              ] else if (leaveHistory.isNotEmpty) ...[
                Text(
